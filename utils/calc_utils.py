...
"""
1.先获取，查询样本和检索集样本的标签匹配情况。（查询正确的样本）
        计算查询样本和检索集的相似度矩阵
        相似度矩阵与0比较，如果大于0则为True
        并转为float型
2.计算查询集和检索集的汉明距离，并对其进行排序，获取排序后的索引---ind
3.将第1步获取的结果按照第2步的索引进行重新排序---gnd
4.获取count数字序列（1，total = min(k, int(tsum))+1）
  获取正例样本在排序结果中的位置
5.计算P精确度=count/tind
  计算平均精确度----map += torch.mean(count / tindex)
6.计算mAP:map = map / num_query
"""
"""
1.计算标签相关性
2.计算相关样本总数
3.计算汉明距离，并排序
4.根据第3步的排序索引，重排相关性标签
5.选择前k个相关样本---计算其索引序列count和实际索引tindex
6.计算平均精度，累积到map中
7.最终将所有查询样本的平均精度取平均---map
"""
def calc_map_k_matrix(qB, rB, query_L, retrieval_L, k=None, rank=0):
    ...
    map = 0
    if k is None:
        k = retrieval_L.shape[0] # 默认情况下，k为检索集大小
    for iter in range(num_query):
        # 获取查询样本的标签---查询样本与检索集样本的标签匹配情况，匹配为1.0
        gnd = (query_L[iter].unsqueeze(0).mm(retrieval_L.t()) > 0).type(torch.float).squeeze()
        tsum = torch.sum(gnd) # 相关标签的总数（查对了的数量）
        # 如果没有相关标签，则跳过该查询样本
        if tsum == 0:
            continue
        hamm = calc_hammingDist(qB[iter, :], rB)
        # 对汉明距离进行排序
        _, ind = torch.sort(hamm) # ind 是排序后的索引------如果 hamm 是 [3, 1, 2]，则 ind 会是 [1, 2, 0]
        ind.squeeze_()
        # 根据排序结果重新排列标签
        gnd = gnd[ind]
        # 获取用于计算mAP的检索结果数量
        ...
        # 获取正例的索引----tindex 中的每个值表示一个相关检索样本在排序结果中的位置。
