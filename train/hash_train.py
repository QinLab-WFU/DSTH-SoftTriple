from model.hash_model import DSTHModel as DSTHModel
import os
...
from utils.loss import SoftTriple
from utils.SPQLoss import SPQLoss
from utils.relative_similarity import *
...
class Trainer(TrainBase):
    def __init__(self):
        args = get_args()
        self.rank = args.rank
        super(Trainer, self).__init__(args, self.rank)
    ...
    def _init_model(self):
        ...
        HashModel = DSTHModel
        self.criterion = SoftTriple(la=0.01, gamma=0.2, tau=0.2, margin=0.5, dim=self.args.output_dim, cN=21, K=5, vi=1.2, device=self.rank).to(self.rank)

        self.relative_similarity = RelativeSimilarity(nbit=self.args.output_dim, nclass=21, batchsize=self.args.batch_size, device=self.rank).to(self.rank)
        self.SPQLoss = SPQLoss(multiclass=True).to(self.rank)
        self.model = HashModel(outputDim=self.args.output_dim, clipPath=self.args.clip_path, writer=self.writer, logger=self.logger, is_train=self.args.is_train).to(self.rank)
        ...
        self.optimizer = BertAdam([
            ...
            {"params": self.relative_similarity.parameters(), "lr": self.args.lr}],
    ...
    def _init_dataset(self):
        ...
        self.args.index_file = os.path.join("/home/virtue/Han_project/dataset", self.args.dataset, self.args.index_file)
        self.args.caption_file = os.path.join("/home/virtue/Han_project/dataset", self.args.dataset, self.args.caption_file)
        self.args.label_file = os.path.join("/home/virtue/Han_project/dataset", self.args.dataset, self.args.label_file)
    ...
    def train_epoch(self, epoch):
        ...
        for image, text, label, index in self.train_loader:
            ...
            softTriple_loss = self.criterion(images=hash_img, texts=hash_text, target=label)
            img = F.normalize(hash_img)
            txt = F.normalize(hash_text)
            logits_i = self.relative_similarity(img)
            logits_t = self.relative_similarity(txt)
            loss_I = self.SPQLoss(logits_i, label)
            loss_T = self.SPQLoss(logits_t, label)
            SPQLoss = loss_I + loss_T
            loss = softTriple_loss + self.args.alpha * SPQLoss
            all_loss += loss
    ...
    def valid(self, epoch):
        ...
        if self.max_mapi2t < mAPi2t:
            ...
            self.save_mat(query_img, query_txt, retrieval_img, retrieval_txt, mode_name="i2t", seed=self.args.seed)
        self.max_mapi2t = max(self.max_mapi2t, mAPi2t)
        if self.max_mapt2i < mAPt2i:
            self.best_epoch_t = epoch
            self.save_mat(query_img, query_txt, retrieval_img, retrieval_txt, mode_name="t2i", seed=self.args.seed)
        self.max_mapt2i = max(self.max_mapt2i, mAPt2i)
        self.logger.info(
    ...
    def save_mat(self, query_img, query_txt, retrieval_img, retrieval_txt, mode_name="i2t", seed=None):
