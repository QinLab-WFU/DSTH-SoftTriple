...
from model.hash_model import DCMHT as DSTH
import os
...
class Trainer(TrainBase):
    ...
    def _init_model(self):
        ...
        HashModel = DSTH
        self.criterion = SoftTriple().to(self.rank)
        self.relative_similarity = RelativeSimilarity().to(self.rank)
        self.SPQLoss = SPQLoss().to(self.rank)
    ...
    def _init_dataset(self):
        ...
        self.args.index_file = os.path.join("/home/user/Projects/dataset", self.args.dataset, self.args.index_file)
        self.args.caption_file = os.path.join("/home/user/Projects/dataset", self.args.dataset, self.args.caption_file)
        self.args.label_file = os.path.join("/home/user/Projects/dataset", self.args.dataset, self.args.label_file)
    ...
    def train_epoch(self, epoch):
        ...
        for image, text, label, index in self.train_loader:
            ...
            Loss_st = self.criterion(images=hash_img, texts=hash_text, target=label)
            Loss_spq = self.SPQLoss(self.relative_similarity(img), self.relative_similarity(txt), label)

            loss = Loss_st + self.args.alpha * Loss_spq
